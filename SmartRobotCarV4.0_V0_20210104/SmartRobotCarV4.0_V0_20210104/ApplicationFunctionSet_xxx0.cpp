...
void ApplicationFunctionSet::ApplicationFunctionSet_Init(void)
{
  ...
  AppServo.DeviceDriverSet_Servo_Init(SERVO_CENTER_POSITION);
  ...
  diagnostics.logMessage(DIAG_LEVEL_INFO, "INIT", "All systems initialized successfully");
...
*/
/**
 * @brief Updates all sensor data including voltage, ultrasonic, and line tracking sensors
 * @details This function is called periodically to read sensor values and update global states
 *          - Voltage monitoring with low battery detection
 *          - Ultrasonic distance measurement
 *          - Line tracking sensor array
 * @note Should be called every loop cycle for real-time sensor data
 */
    ...
    if (function_xxx(get_Distance, 0, 20))
    {
      ...
      for (int i = 1; i < 6; i += 2) //1、3、5 Omnidirectional detection of obstacle avoidance status
      {
        ...
        if (function_xxx(get_Distance, 0, 20))
        {
          ...
          if (5 == i)
          {
            ApplicationFunctionSet_SmartRobotCarMotionControl(Backward, MOTOR_DEFAULT_SPEED);
            _delay(500);
            ApplicationFunctionSet_SmartRobotCarMotionControl(Right, MOTOR_DEFAULT_SPEED);
            _delay(100);
            first_is = true;
            break;
          }
        ...
        else
        {
          ...
          case 1:
            ...
            break;
          case 3:
            ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, MOTOR_DEFAULT_SPEED);
            break;
          case 5:
            ApplicationFunctionSet_SmartRobotCarMotionControl(Left, MOTOR_DEFAULT_SPEED);
            break;
          }
          _delay(100);
          first_is = true;
...
/**
 * @brief Parses incoming serial data for robot control commands
 * @details Processes JSON-formatted commands received via serial port
 *          - Validates JSON format with proper error handling
 *          - Supports multiple command types (motor, servo, lighting, etc.)
 *          - Includes watchdog reset to prevent timeout during reception
 * @note Commands must be properly formatted JSON ending with '}'
 */
void ApplicationFunctionSet::ApplicationFunctionSet_SerialPortDataAnalysis(void)
{
  ...
  if (Serial.available() > 0)
  {
    while (c != '}' && Serial.available() > 0)
    {
      ...
      wdt_reset(); // Prevent watchdog timeout during long serial reception
    }
  }
  if (c == '}') //数据帧尾部校验
  {
    ...
    StaticJsonDocument<JSON_BUFFER_SIZE> doc;                          //声明一个JsonDocument对象
    DeserializationError error = deserializeJson(doc, SerialPortData); //反序列化JSON数据
    if (error)
    {
      diagnostics.logMessage(DIAG_LEVEL_ERROR, "JSON", "Parse error occurred");
    ...
    else if (!error) //检查反序列化是否成功
    {
      ...
      case 200: /*<命令：N 200> System Diagnostics */
        {
          int diag_command = doc["D1"];
          switch(diag_command) {
            case 1: // System status
              diagnostics.printSystemStatus();
              break;
            case 2: // Performance stats
              diagnostics.printPerformanceStats();
              break;
            case 3: // Reset error counters
              diagnostics.resetErrorCounters();
              break;
            default:
              diagnostics.logMessage(DIAG_LEVEL_WARNING, "DIAG", "Unknown diagnostic command");
              break;
          }
#if _is_print
          Serial.print('{' + CommandSerialNumber + "_ok}");
      ...
      default:
        diagnostics.logMessage(DIAG_LEVEL_WARNING, "CMD", "Unknown command received");
        break;
      }
    }
  }
