...
// Smooth motor control wrapper - applies gradual acceleration
static void SmoothMotorControl(boolean dir_A, uint8_t speed_A, boolean dir_B, uint8_t speed_B) {
  uint8_t actualSpeed_A = speed_A;
  uint8_t actualSpeed_B = speed_B;

  // Apply smooth acceleration/deceleration
  smoothMotion.setTargetSpeed(speed_A, speed_B);
  smoothMotion.update(actualSpeed_A, actualSpeed_B);

  // Call actual motor control with smoothed speeds
  AppMotor.DeviceDriverSet_Motor_control(dir_A, actualSpeed_A, dir_B, actualSpeed_B, control_enable);
}
...
static void ApplicationFunctionSet_SmartRobotCarMotionControl(SmartRobotCarMotionControl direction, uint8_t is_speed)
{
  ...
  // Obstacle avoidance: reduce speed when approaching obstacles (only for forward motion)
  if (direction == Forward && g_UltrasoundData_cm > 0 && g_UltrasoundData_cm < 400) {
    speed = smoothMotion.adjustForObstacle(g_UltrasoundData_cm, speed);
    if (speed == 0) {
  ...
  case /* constant-expression */ Left:
    /* code */
    directionRecord = 3;
    SmoothMotorControl(/*direction_A*/ direction_just, /*speed_A*/ speed,
                       /*direction_B*/ direction_back, /*speed_B*/ speed); //Motor control with smooth acceleration
    break;
  case /* constant-expression */ Right:
    /* code */
    directionRecord = 4;
    SmoothMotorControl(/*direction_A*/ direction_back, /*speed_A*/ speed,
                       /*direction_B*/ direction_just, /*speed_B*/ speed); //Motor control with smooth acceleration
    break;
  case /* constant-expression */ LeftForward:
    /* code */
    directionRecord = 5;
    SmoothMotorControl(/*direction_A*/ direction_just, /*speed_A*/ speed,
                       /*direction_B*/ direction_just, /*speed_B*/ speed / 2); //Motor control with smooth acceleration
    break;
  case /* constant-expression */ LeftBackward:
    /* code */
    directionRecord = 6;
    SmoothMotorControl(/*direction_A*/ direction_back, /*speed_A*/ speed,
                       /*direction_B*/ direction_back, /*speed_B*/ speed / 2); //Motor control with smooth acceleration
    break;
  case /* constant-expression */ RightForward:
    /* code */
    directionRecord = 7;
    SmoothMotorControl(/*direction_A*/ direction_just, /*speed_A*/ speed / 2,
                       /*direction_B*/ direction_just, /*speed_B*/ speed); //Motor control with smooth acceleration
    break;
  case /* constant-expression */ RightBackward:
    /* code */
    directionRecord = 8;
    SmoothMotorControl(/*direction_A*/ direction_back, /*speed_A*/ speed / 2,
                       /*direction_B*/ direction_back, /*speed_B*/ speed); //Motor control with smooth acceleration
    break;
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
{
  ...
  { /*避障状态更新*/
    ...
    g_UltrasoundData_cm = UltrasoundData_cm; // Update global for obstacle avoidance
