...
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Get(uint16_t *ULTRASONIC_Get /*out*/)
{
  ...
  unsigned long duration = pulseIn(ECHO_PIN, HIGH, ULTRASONIC_TIMEOUT_MS * 1000); // Convert ms to microseconds
  if (duration == 0)
  {
    // Sensor timeout - return safe default
    *ULTRASONIC_Get = ULTRASONIC_MAX_DISTANCE; // Safe default - assume obstacle far away
    return;
  }

  tempda_x = duration / 58;

  // Validate sensor reading range
  if (tempda_x < ULTRASONIC_MIN_DISTANCE)
  {
    // Too close reading, likely sensor error
    *ULTRASONIC_Get = ULTRASONIC_MIN_DISTANCE; // Minimum valid distance
  }
  else if (tempda_x > ULTRASONIC_MAX_DISTANCE)
  {
    *ULTRASONIC_Get = ULTRASONIC_MAX_DISTANCE; // Maximum detection range
  }
  else
  {
